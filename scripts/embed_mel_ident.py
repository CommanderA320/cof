...
from mel_chunker import chunk_mel, FLEET
...
def ensure_ata_ref_index():
  try:
    ...
    print('ata_ref index ensured')
  except Exception as e:
    print(f'ata_ref index note: {e}')
...
t1 = time.time()
chunks = chunk_mel(text, doc_code=DOC_CODE)
...
for c in chunks:
  ...
  if c['section'] == 'item':
    ...
    if c['ata_ref'] and c['ata_ref'] != 'UNKNOWN':
      ata_filled += 1

item_count = sections.get('item', 0)
print(f'Sections: {sections}')
print(f'Applicability kinds (item only): {kinds}')
print(f'ata_ref filled: {ata_filled}/{item_count}')

print(f'Deleting existing {DOC_CODE} points...')
  ...
  points = [
    PointStruct(
      ...
      payload={
        ...
        'applicability_kind': c['applicability_kind'],
        'applicability_tails': c['applicability_tails'],
      }
    )
